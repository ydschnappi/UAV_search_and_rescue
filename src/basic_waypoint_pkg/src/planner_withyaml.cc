...
BasicPlanner::BasicPlanner(ros::NodeHandle& nh) :
    ...
    if (!nh_.getParam(ros::this_node::getName() + "/dynamic_params/max_v", max_v_)){
        ROS_WARN("[BasicPlanner] param max_v not found");
    }
    if (!nh_.getParam(ros::this_node::getName() + "/dynamic_params/max_a", max_a_)){
        ROS_WARN("[BasicPlanner] param max_a not found");
    }
...
bool BasicPlanner::planTrajectory(const Eigen::VectorXd& goal_pos,
    ...
    if (!nh_.getParam(ros::this_node::getName() + "/waypoints", waypoints)){
        ROS_WARN("[BasicPlanner] waypoints not found");
    }
    // for(size_t i = 0;i<waypoints.size();++i){
    //     Eigen::Vector3d position;
    //     const double& x_pos = waypoints[i]["x"];
    //     const double& y_pos = waypoints[i]["y"];
    //     const double& z_pos = waypoints[i]["z"];
    //     ROS_INFO("%f", x_pos);
    ...
    if (waypoints.getType() == XmlRpc::XmlRpcValue::TypeArray)
    {
        ...
        {
            Eigen::Vector3d position;
            double x_pos = waypoints[i][0];
            double y_pos = waypoints[i][1];
            double z_pos = waypoints[i][2];
            ROS_INFO("%f", x_pos);
            position << x_pos,y_pos,z_pos;
            middle.addConstraint(mav_trajectory_generation::derivative_order::POSITION,position);
            vertices.push_back(middle);
    ...
    // const int segments_num = 6;
    // for(int i=1;i<=segments_num;i++){
    //     std::string str = "/planner/waypoints";
    //     std::stringstream s;
    //     s<<i;
    //     str.append(s.str());
    //     std::vector <double> waypoint;
    //     nh_.getParam(ros::this_node::getName() + str, waypoint);
    //     Eigen::Vector3d position;
    //     position << waypoint[1], waypoint[2], waypoint[3];
    //     middle.addConstraint(mav_trajectory_generation::derivative_order::POSITION,position);
    //     vertices.push_back(middle);
    // }
    //
